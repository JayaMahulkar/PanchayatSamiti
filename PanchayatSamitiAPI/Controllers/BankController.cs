...
namespace PanchayatSamitiAPI.Controllers
{
    ...
    public class BankController : ControllerBase
    {
        ...
        [HttpPost]
        [Route("Create")]
        public async Task<IActionResult> Create([FromBody] BankModel model)
        {
            if (model == null) return BadRequest();
            var result = await _service.AddBankAsync(model);
            if (result) return Ok();
        ...
        [HttpPut]
        [Route("Update")]
        public async Task<IActionResult> Update([FromBody] BankModel model)
        {
            if (model == null) return BadRequest();
